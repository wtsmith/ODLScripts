...
UPDATE ODL_Raw O
	...
	SET O.Series = N.Series,  O.Family = N.Family, O.DoorWindow = N.DoorWindow;
...
UPDATE ODL_Raw SET Manufacturer = 
	CASE Manufacturer_Code
		...
		WHEN 'BSS' THEN 'Builders Specialty Supply, Therma-Tru'
...
UPDATE odl_raw SET MPC = replace( replace(MPC, ',', ''), '"', '' ),
	Series = replace( replace(Series, ',', ''), '"', '' ),
	Series_Concat = replace( replace(Series_Concat, ',', ''), '"', '' ),
	Subgroup = trim(Subgroup);

-- Apply Energy Star Ratings

UPDATE odl_raw
SET es_northern = 1, es_northcentral = 2, es_southcentral = 3, es_southern = 4
where doorwindow = 'door' and mpc_parsed_ltsize = 'None' and u_factor <=0.17;

UPDATE odl_raw
SET es_northern = 1, es_northcentral = 2, es_southcentral = 3, es_southern = 4
where doorwindow = 'door' and mpc_parsed_ltsize IN('1/4', '1/2') and u_factor <=0.25 and shgc <=0.25;

UPDATE odl_raw
SET es_northern = 1, es_northcentral = 2
where doorwindow = 'door' and mpc_parsed_ltsize IN('3/4', 'Full') and u_factor <=0.3 and shgc <=0.4;

UPDATE odl_raw
SET es_southcentral = 3, es_southern = 4
where doorwindow = 'door' and mpc_parsed_ltsize IN('3/4', 'Full') and u_factor <=0.3 and shgc <=0.25;

UPDATE odl_raw
SET es_northern = 1
where doorwindow = 'window' and u_factor <=0.3;

UPDATE odl_raw
SET es_northern = 1
where doorwindow = 'window' and u_factor <= 0.31 and shgc >= 0.35 and es_northern = 0;

UPDATE odl_raw
SET es_northern = 1
where doorwindow = 'window' and u_factor <= 0.32 and shgc >= 0.4 and es_northern = 0;

UPDATE odl_raw
SET es_northcentral = 2
where doorwindow = 'window' and u_factor <= 0.3 and shgc <= 0.4;

UPDATE odl_raw
SET es_southcentral = 3
where doorwindow = 'window' and u_factor <= 0.3 and shgc <= 0.25;

UPDATE odl_raw
SET es_southern = 4
where doorwindow = 'window' and u_factor <= 0.4 and shgc <= 0.25;

update odl_raw
set es_zones
= concat(es_northern, es_northcentral, es_southcentral, es_southern)