...
-- Below modified on June 25th, 2014.  Added MPC_Parsed lit size filter and parse of MPC to capture 'BLINDS'.  Not perfect yet, but moving in that direction.
UPDATE ODL_Raw o
INNER Join decoder_ring d ON o.GL = d.GL 
	AND o.Spacer = d.Spacer 
	AND o.Gap_Width_Str1 >= d.Gap_Width_Str1 - 0.03 AND o.Gap_Width_Str1 <= d.Gap_Width_Str1 + 0.03
	AND o.Gap_Width_Str2 >= d.Gap_Width_Str2 - 0.03 AND o.Gap_Width_Str2 <= d.Gap_Width_Str2 + 0.03
	AND (
		o.MPC_Parsed_LtSize = IF(d.Door_Lite_Size_14 = 'X', '1/4', 'Poo') OR
		o.MPC_Parsed_LtSize = IF(d.Door_Lite_Size_12 = 'X', '1/2', 'Poo') OR
		o.MPC_Parsed_LtSize = IF(d.Door_Lite_Size_34 = 'X', '3/4', 'Poo') OR
		o.MPC_Parsed_LtSize = IF(d.Door_Lite_Size_Full = 'X', 'Full', 'Poo') 
		) 
	AND d.Glass_Type LIKE IF(MPC RLIKE '^.*BLINDS.*$', 'Blinds', '%')
SET 	o.Glass_Type = d.Glass_Type,
	o.Subgroup = d.Subgroup,
	o.Door_Lite_Size_14 = d.Door_Lite_Size_14,
	o.Door_Lite_Size_12 = d.Door_Lite_Size_12,
	o.Door_Lite_Size_34 = d.Door_Lite_Size_34,	
	o.Door_Lite_Size_Full = d.Door_Lite_Size_Full;
	
/*
-- Added Aug 9, 2015.  See email from Jim A SW Blinds and Commodity Aug 5 2015
-- Don't really like hard coding things this way, but oh well.
UPDATE odl_raw o
SET o.glass_type = 'Severe Weather, Impact-rated', o.subgroup = 'Severe Weather Blind'
WHERE glass_type = 'Blinds'  AND
	(mpc rlike '^.*LAMI?.*$' OR	mpc rlike '^.*IMP.*$');
*/ --  Taken out August 22 when impoemented new search tool using update statements.	
-- -------After joining decoder ring.
